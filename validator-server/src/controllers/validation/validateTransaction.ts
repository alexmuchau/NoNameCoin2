...
export async function validateTransaction(req: any, res: any) {
    ...
    if (senderTransactionsCount > 1000) {
        ...
        res.send({validation: "DENIED", transId: transaction.trans_id, validatorId: validatorId})
        printFooter("Erro na validação: limite de transações excedido")
        return
    }
    
    if (transaction.trans_coins + transaction.trans_tax > transaction.sender.coins_in_stock) {
        // await sendValidation(transaction.trans_id, validatorId, "DENIED")
        res.send({validation: "DENIED", transId: transaction.trans_id, validatorId: validatorId})
    ...
    res.send({validation: "APPROVED", transId: transaction.trans_id, validatorId: validatorId})
