...
async function validateTransaction(transaction: any, validators: ReducedValidatorsType[], senderTransactionsCount: number) {
    ...
    for (const validator of validators) {
        ...
        .then((res) => res.json())
        .then(async (res: { validation: "APPROVED" | "DENIED", transId: string, validatorId: string }) => {
    ...
    if (validators.length == validations.APPROVED.length + validations.DENIED.length) {
